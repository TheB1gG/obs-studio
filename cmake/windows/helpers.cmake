...
function(_bundle_dependencies target)
  ...
  # x265 is a runtime dependency of FFmpeg avcodec when HEVC encoding is enabled,
  # but has no CMake target so it is not discovered by find_dependencies()
  if(ENABLE_HEVC AND TARGET FFmpeg::avcodec)
    get_target_property(_avcodec_loc FFmpeg::avcodec IMPORTED_LOCATION_RELEASE)
    if(NOT _avcodec_loc)
      get_target_property(_avcodec_loc FFmpeg::avcodec IMPORTED_LOCATION)
    endif()
    if(_avcodec_loc)
      get_filename_component(_ffmpeg_bin_dir "${_avcodec_loc}" DIRECTORY)
      find_program(_x265_dll NAMES libx265.dll HINTS "${_ffmpeg_bin_dir}" NO_DEFAULT_PATH)
      if(_x265_dll)
        foreach(config IN ITEMS DEBUG RELWITHDEBINFO RELEASE MINSIZEREL)
          list(APPEND library_paths_${config} "${_x265_dll}")
        endforeach()
      endif()
    endif()
    unset(_avcodec_loc)
