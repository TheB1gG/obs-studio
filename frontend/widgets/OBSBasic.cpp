...
OBSBasic::OBSBasic(QWidget *parent) : OBSMainWindow(parent), undo_s(ui), ui(new Ui::OBSBasic)
{
	...
	connect(controls, &OBSBasicControls::KillStreamButtonClicked, this, &OBSBasic::KillStreamingUngracefully);
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	config_set_default_bool(activeConfiguration, "Stream1", "MultitrackVideoRequestMaxTracks", false);
...
static inline enum obs_scale_type GetScaleType(ConfigFile &activeConfiguration)
{
	...
	else if (astrcmpi(scaleTypeStr, "bilinear_lowres") == 0)
		return OBS_SCALE_BILINEAR_LOWRES;
	else if (astrcmpi(scaleTypeStr, "integer_area") == 0)
