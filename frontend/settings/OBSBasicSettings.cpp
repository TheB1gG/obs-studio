...
#undef ADD_HOTKEY_FOCUS_TYPE
	...
	addScaleFilter("Basic.Settings.Video.DownscaleFilter.BilinearLowQuality", OBS_SCALE_BILINEAR_LOWRES);
	addScaleFilter("Basic.Settings.Video.DownscaleFilter.IntegerArea", OBS_SCALE_INTEGER_AREA);
...
void OBSBasicSettings::LoadDownscaleFilters()
{
	...
	if (ui->baseResolution->currentText() == ui->outputResolution->currentText()) {
		...
		ui->downscaleFilter->addItem(QTStr("Basic.Settings.Video.DownscaleFilter.BilinearLowQuality"),
					     QT_UTF8("bilinear_lowres"));
		ui->downscaleFilter->addItem(QTStr("Basic.Settings.Video.DownscaleFilter.IntegerArea"),
				     QT_UTF8("integer_area"));
		...
		else if (downscaleFilter == "bilinear_lowres")
			ui->downscaleFilter->setCurrentIndex(5);
		else if (downscaleFilter == "integer_area")
			ui->downscaleFilter->setCurrentIndex(6);
		else
...
void OBSBasicSettings::UpdateMultitrackVideo()
{
	...
	// Developer mode is now always enabled (the --enable-multitrack-video-dev flag was removed),
	// so these checks no longer restrict availability; they are kept for compatibility.
